...
export default class ReactImageGallerys extends React.Component {
  ...
  _updateThumbnailTranslate(previousIndex) {
    const { thumbsTranslate, currentIndex,thumbnailsTranslate } = this.state;
    ...
    if (this.state.currentIndex === 0) {
      ...
      if (scroll > 0) {
        if (previousIndex < currentIndex) {
          this._setThumbsTranslate(thumbsTranslate - scroll-thumbnailsTranslate);
  ...
  _setThumbsTranslate(thumbsTranslate,thumbnailsTranslate=0) {
    this.setState({
      thumbsTranslate:thumbsTranslate,
      thumbnailsTranslate:thumbnailsTranslate
    });
  }
  ...
  _onThumbnailNavClick=(event,isLeft=false)=> {
      //let scroll = this._getThumbsTranslate(7);
      const { thumbsTranslate,thumbnailsTranslate,thumbnailsWrapperWidth } = this.state;
      if(isLeft){
        let num = (thumbsTranslate+thumbnailsWrapperWidth)>0?0:thumbsTranslate+thumbnailsWrapperWidth;
        this._setThumbsTranslate(num,thumbnailsTranslate + (num-thumbsTranslate));
      }else {
        let scrolNum =thumbnailsWrapperWidth-this._thumbnails.scrollWidth;
        let num = (thumbsTranslate-thumbnailsWrapperWidth)<scrolNum?scrolNum:thumbsTranslate-thumbnailsWrapperWidth;
        this._setThumbsTranslate(num,thumbnailsTranslate - (thumbsTranslate-num));
      }
  }
  render() {
    const {
    ...
    const imageThumbnailNav = (
        <div>
          <button type="button"
                  onClick={event => this._onThumbnailNavClick(event,true)}
                  className="image-gallery-left-nav image-thumbnails-left-nav"/>
          <button type="button"
                  onClick={event => this._onThumbnailNavClick(event,false)}
                  className="image-gallery-right-nav image-thumbnails-right-nav"/>
        </div>
    )
    return (
      <div
          ...
          }
          {
            this.props.showThumbnails &&
              ...
              >
                ...
                >
                  {this.props.thumbnailPosition === 'bottom'?imageThumbnailNav:null}
                  <div
